...
### 8.64 `/creators` redirects to the Afterhours creator landing (Afterhours §5.171 ONEFORM, 4 Sep 2026)

`next.config.ts` gains a `redirects()` entry: `/creators` → `https://www.onlyafterhours.com/creators`, `permanent: false`. It exists so the creator onboarding Instagram post can obey §8.49's CTA law (SP-side creative always points at `sonicpulsefestival.com`) while landing people on the Afterhours page that signs them in, flips creator mode, and opens the one five-field form already pointed at the Sonic Pulse brief. Temporary (307) on purpose: the destination may later become an SP-hosted page, and a 308 would be cached by browsers for the life of the post.

Spec, copy, acceptance and the harness live in `afterhours/plan-afterhours.md` §5.171.8 (this file's part is the verbatim `next.config.ts` there); the built order is Afterhours `main` first, then this repo — the redirect must never ship before its target exists. Build task is Afterhours #411. The change touches only `next.config.ts` and this entry.

### 8.65 `/register` — the buyer post's doorway, and the last stale copy dies (Afterhours §5.173 GUESTLIST, 4 Sep 2026)
